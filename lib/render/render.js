...
// ─────────────────────────────────────────────────────────────
// Tamanhos de logo no header (altura máxima)
const LOGO_HEIGHT = { S: '32px', M: '44px', L: '56px' };

function renderHeader(dna, logoTxt, pilar) {
  const estilo = dna.header.estilo;

  // Se existir asset_url, usa <img>; senão usa wordmark texto
  const logoInner = dna.logo?.asset_url
    ? `<img src="${dna.logo.asset_url}" alt="${escapeHtml(logoTxt)}" style="height:${LOGO_HEIGHT[dna.logo.tamanho_header] || '44px'}; width:auto; display:block; object-fit:contain;">`
    : escapeHtml(logoTxt);
  const logo = `<div class="logo">${logoInner}</div>`;
