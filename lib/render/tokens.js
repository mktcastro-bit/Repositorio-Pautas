...
export function decoracaoHtml(dna, { indiceSlide, palavraChave }) {
  ...
  switch (d.tipo) {
    ...
    case 'simbolo-asset':
    case 'padrao-asset': {
      // asset_url é resolvida pelo cliente antes do renderSlide — data URL base64
      if (!d.asset_url) return '';
      const wPx = { S: 120, M: 180, L: 260, XL: 380 }[d.tamanho] || 180;
      const imgStyle = `position:absolute; ${posicao} width:${wPx}px; height:auto; opacity:${opacity}; pointer-events:none; z-index:1; object-fit:contain;`;
      return `<img class="deco" src="${d.asset_url}" alt="" style="${imgStyle}">`;
    }
    default:
      return '';
  }
